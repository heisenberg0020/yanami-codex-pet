...
**v0.2.1 已实现：零食收藏 + 专注采购 + Codex 协作采购。** 可在独立 Mac 窗口或 Codex 内置浏览器打开，二者共享本机存档。可选官方 Hooks 把工作事件送入小卖部，需先审核信任。原生 Codex Mini 保持原版；工作气泡和点心操作显示在小卖部中。本机七条 Hook 已经审核启用；一个真实 CLI 回合加两个由 Codex 应用续接的测试回合，已带回并领取一份草莓牛奶。v0.2.1 修复了续接时缺少提交消息事件导致不计数的问题。安装前已打开的当前开发任务仍未观测到事件，不能推断所有已有桌面任务都已接通。详见[实际验收记录](reviews/codex-integration-v1/review.md)。
